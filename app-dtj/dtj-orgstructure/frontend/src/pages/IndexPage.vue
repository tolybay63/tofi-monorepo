<template>
  <q-page class="q-pa-md">
    ...
    <q-btn dense no-caps color="primary" label="Загрузка объектов" @click="loadLocation"/>
    <div> {{ this.rows }}</div>
    <hr class="q-mb-xl"/>

    <q-btn dense no-caps label="Добавить" color="primary" @click="saveLocation('ins')"/>
    <div> {{ this.row }}</div>
    <hr class="q-mb-xl"/>

    <div class="row">
      <q-input :model-value="id" v-model="id" label="id записи" type="number" :disable="rows.length===0"></q-input>
      <q-btn no-caps :disable="rows.length===0" dense class="q-ml-md" label="Редактировать" color="primary" @click="saveLocation('upd')"/>
    </div>
    <div> {{ this.row }}</div>
