...
import UpdaterFishGearMeters from "@/pages/fishgear/UpdaterFishGearMeters.vue";
...
const fnDelete = (row) => {
  ...
  $q.dialog({
    ...
    .onOk(() => {
      api.post('', {
        method: "data/deleteFishGearMeters",
        params: [row.idval],
      })
        .then(() => {
...
const updateRowValue = (currentRow, targetRec) => {
  let row = findRowForId(rows.value, currentRow.id)
  if (row) {
    row.idval = targetRec.id
    row.numberval = targetRec.value
  }
}
...
const fnEdit = (row) => {
  ...
  $q.dialog({
    component: UpdaterFishGearMeters,
    componentProps: {
      data: rec,
    },
  })
    .onOk((r) => {
      updateRowValue(row, r)
    })
    .onCancel(() => {
      notifyInfo(proxy?.$t("canceled"))
    })
...
const clearFishGearData = () => {
  rows.value = []
}

const loadFishGearMeters = (targetObj) => {
  loading.value = true
  obj.value = targetObj
  api.post('', {
    method: 'data/loadFishGearMeters',
    params: [targetObj],
  })
    .then((response) => {
