<template>
  <span class="q-pa-sm-sm">
    ...
    <span v-if="checked_visible">
      <span style="color: #1976d2; margin-left: 5px"> {{ $t('selectedNode') }}: </span>
      {{ this.nodeInfo() }}
    </span>
  </span>
          ...
          <td v-for="(col, i) in cols_" :data-th="col.name" :key="i">
            <span v-if="col.checked && col.checked === 'true'">
              <q-btn
                ...
                flat
                color="blue"
                :icon="item[col.field] ? 'check_box' : 'check_box_outline_blank'"
              >
              </q-btn>
            </span>
            <span v-else>
...
/*    clrAny() {
    ...
    fnDPT(item) {
      return this.FD_DimPropType ? this.FD_DimPropType.get(item["dimPropType"]) : null
    },

    fnPT(item) {
      let at = ''
      if (item["propType"] === allConsts.FD_PropType.attr) {
        if (item["attribValType"] === allConsts.FD_AttribValType.str) at = ' (' + this.$t('str') + ')'
...
/*    restoreSelect(item) {
    ...
    selectedRow(item) {
      let vm = this
      if (vm.selected.length > 0 && item.id === vm.selected[0].id) vm.selected = []
      else {
        vm.selected = []
        vm.selected.push(item)
      }
