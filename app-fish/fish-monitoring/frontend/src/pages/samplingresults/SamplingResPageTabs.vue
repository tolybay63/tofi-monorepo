  ...
  <div class="q-pa-sm-sm bg-green-1">
    ...
    <q-tab-panels v-model="tab" animated>
      <q-tab-panel :name="fnNameTab(0)" class="no-scroll">
        <SamplingResTabs codCls="Cls_SamplingFishing" :cls="fnClsTab(0)" />
      </q-tab-panel>

      <q-tab-panel :name="fnNameTab(1)" class="no-scroll">
        <SamplingResTabs codCls="Cls_SamplingZooplankton" :cls="fnClsTab(1)" />
      </q-tab-panel>

      <q-tab-panel :name="fnNameTab(2)" class="no-scroll">
        <SamplingResTabs codCls="Cls_SamplingZoobenthos" :cls="fnClsTab(2)" />
      </q-tab-panel>

      <q-tab-panel :name="fnNameTab(3)" class="no-scroll">
        <SamplingResTabs codCls="Cls_SamplingHydrophysical" :cls="fnClsTab(3)" />
      </q-tab-panel>

      <q-tab-panel :name="fnNameTab(4)" class="no-scroll">
        <SamplingResTabs codCls="Cls_SamplingHydrological" :cls="fnClsTab(4)" />
      </q-tab-panel>

      <q-tab-panel :name="fnNameTab(5)" class="no-scroll">
        <SamplingResTabs codCls="Cls_SamplingHydrochemistry" :cls="fnClsTab(5)" />
      </q-tab-panel>
    </q-tab-panels>
  </div>
</template>
...
import {ref} from 'vue'
import SamplingResTabs from 'pages/samplingresults/SamplingResTabs.vue'

export default {
  name: 'SamplingResPageTabs',
  components: { SamplingResTabs },

  data: function () {
    return {
      loading: false,
      rows: [],
      tab: ref('Cls_ResSamplingFishing'),
    }
  },
  ...
  methods: {
    ...
    loadResSamplingCls() {
      ...
      api
        ...
        .then(() => {
          this.tab = ref('Cls_ResSamplingFishing')
        })
        .catch((error) => {
