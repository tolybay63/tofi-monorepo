...
import { notifyError, notifySuccess, pack, today } from '@/utils/jsutils'
...
const onOKClick = () => {
  ...
  form.objBranch = objBranch.value
...
onMounted(() => {
  ...
  api.post('', { method: 'data/loadBranchForSelect', params: ['Prop_Branch'] }).then((res) => {
    optBranch.value = res.data.result.records
  })
  ...
  api
    ...
    .then((res) => {
      ...
      if (props.mode === 'upd') {
        let arr = props.data.objBranch?.split(',') || []
        arr.forEach((item) => objBranch.value.push(item))

        arr = props.data.objKATO?.split(',') || []
