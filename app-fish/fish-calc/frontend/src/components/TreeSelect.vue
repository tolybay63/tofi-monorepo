...
// Функция для безопасного извлечения скалярного значения (id/key), если передан объект
const extractKeyValue = (val) => {
  if (val && typeof val === 'object') {
    return val[props.nodeKey] ?? null
  }
  return val
}

const selectedSingle = ref(props.multiple ? null : extractKeyValue(props.modelValue))
const selectedMultiple = ref(props.multiple ? (Array.isArray(props.modelValue) ? props.modelValue.map(extractKeyValue) : []) : [])

watch(
  () => props.modelValue,
  (newVal) => {
    if (props.multiple) {
      selectedMultiple.value = Array.isArray(newVal) ? newVal.map(extractKeyValue) : []
    } else {
      selectedSingle.value = extractKeyValue(newVal)
    }
  },
  { deep: true }
)
