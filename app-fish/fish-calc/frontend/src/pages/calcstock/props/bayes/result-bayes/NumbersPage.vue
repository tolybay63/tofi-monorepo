...
const loadResultNumbersNotPeriod = (objId) => {
  if (!objId) return;
  loading2.value = true
  api
    .post('', {
      method: 'data/loadResultNumbersNotPeriod',
      params: [objId],
    })
    .then((response) => {
...
const loadResultNumbers = (objId) => {
  if (!objId) return;
  loading.value = true
  api
    .post('', {
      method: 'data/loadResultNumbers',
      params: [objId],
    })
    .then((response) => {
...
watch(
  () => props.own,
  (newObj) => {
    loadResultNumbers(newObj)
    loadResultNumbersNotPeriod(newObj)
  },
  {immediate: true}
)
